...
# Eagerly import torch to avoid blocking the event loop on first inference
try:
    import torch
except Exception:
    pass
...
class SilenceHFWarningFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        if "unauthenticated requests" in record.getMessage():
            return False
        return True
...
async def on_ready() -> None:
    ...
    if config.guild_ids:
        # Clear global commands to prevent duplicates between global and guild scopes
        try:
