...
from database import add_modlog, add_scheduled
...
class Moderation(commands.Cog):
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        ...
        case = add_modlog(member.id, "Ban", reason, ctx.author.id)
    ...
    async def tempban(self, ctx, member: discord.Member, duration: str, *, reason: str = "No reason provided"):
        ...
        if unit not in multiplier:
            return await ctx.send(embed=discord.Embed(description="Invalid unit; use s/m/h/d.", color=discord.Color.red()))
        seconds = amount * multiplier[unit]

        await member.ban(reason=reason)
        case = add_modlog(member.id, f"Tempban ({duration})", reason, ctx.author.id)

        execute_ts = int(datetime.now(timezone.utc).timestamp()) + seconds
        add_scheduled(ctx.guild.id, member.id, "unban", execute_ts, None)
    ...
    async def unban(self, ctx, user_id: int, *, reason: str = "No reason provided"):
        ...
        case = add_modlog(user_id, "Unban", reason, ctx.author.id)
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        ...
        case = add_modlog(member.id, "Kick", reason, ctx.author.id)
    ...
    async def mute(self, ctx, member: discord.Member, duration: str = None, *, reason: str = "No reason provided"):
        ...
        if duration:
            ...
            case = add_modlog(member.id, f"Mute ({duration})", reason, ctx.author.id)
            execute_ts = int(datetime.now(timezone.utc).timestamp()) + seconds
            add_scheduled(ctx.guild.id, member.id, "auto-unmute", execute_ts, None)
            ...
            case = add_modlog(member.id, "Mute (Indefinite)", reason, ctx.author.id)
            em = discord.Embed(title="🔇 Muted", color=discord.Color.dark_gray())
            em.add_field(name="User", value=f"{member} ({member.id})", inline=False)
            em.add_field(name="Reason", value=reason, inline=False)
            em.add_field(name="Case", value=f"#{case}", inline=False)
    ...
    async def unmute(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        ...
        if role and role in member.roles:
            ...
            case = add_modlog(member.id, "Unmute", reason, ctx.author.id)
            ...
            await ctx.send(embed=discord.Embed(description="User is not muted.", color=discord.Color.red()))
    ...
    async def timeout(self, ctx, member: discord.Member, duration: str, *, reason: str = "No reason provided"):
        ...
        case = add_modlog(member.id, f"Timeout ({duration})", reason, ctx.author.id)
        execute_ts = int(datetime.now(timezone.utc).timestamp()) + seconds
        add_scheduled(ctx.guild.id, member.id, "untimeout", execute_ts, None)
    ...
    async def warn(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        case = add_modlog(member.id, "Warn", reason, ctx.author.id)
    ...
    async def warnings(self, ctx, member: discord.Member):
        rows = add_modlog.get_modlogs(member.id)
        warns = [r for r in rows if r[2] == "Warn"]
        if not warns:
            await ctx.send(embed=discord.Embed(description="No warnings found.", color=discord.Color.grayple()))
            return
    ...
    async def clearwarns(self, ctx, member: discord.Member):
        import sqlite3
        conn = sqlite3.connect("modlogs.db")
        cur = conn.cursor()
        cur.execute("DELETE FROM modlogs WHERE user_id = ? AND action = ?", (member.id, "Warn"))
        conn.commit()
        conn.close()
